...
import {
  ...
  ActivityIndicator, KeyboardAvoidingView, Platform, StyleSheet, Pressable,
} from "react-native";
...
export function SmartInputSheet({
        ...
        {(phase.type === "preview" || phase.type === "confirming") && (
          <View>
            ...
            {phase.response.domains.map((block, i) => {
              if (i < phase.domainIndex) return null;
  ...
  // Web: render as bottom-anchored overlay (Modal fills full viewport on web)
  if (Platform.OS === "web") {
    if (!visible) return null;
    return (
      <View style={s.webBackdrop} pointerEvents="box-none">
        <Pressable style={s.webBackdropTouchable} onPress={handleClose} />
